...
router.get('/', async (req, res) => {
    try {
        ...
        console.log(error)
        res.status(500).json({ 'message': 'error while getting all the grocery items' })
    }
})
...
router.put('/:groceryItemId',isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        if (groceryItemToUpdate.createdBy == userId) {
            const groceryItemToUpdate = await Book.findByIdAndUpdate(groceryItemId, payload, { new: true })
