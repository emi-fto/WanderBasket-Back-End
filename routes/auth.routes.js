...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    if (!potentialUser) {
      ...
      try {
        await User.create({ email, hashedPassword, username, picture });
        res.status(201).json({ message: "User created" });
      } catch (error) {
        console.log(error);
        console.log("There is an error creating user");
    ...
    console.log(error);
    console.log("There was an error while trying to get a user");
