...
router.get('/:userId', async (req, res) => {
    ...
    try {
        const tripsForSpecificParticipant = Trip.find({ participants: { $in: [participantId] } })
        res.json(tripsForSpecificParticipant);
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: "error while getting the trips for a specific participant" })
    }
})
...
router.put('/: tripId', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        if (tripToUpdate.participants.includes(userId)) { 
            const updatedTrip = await Trip.findByIdAndUpdate(tripId, payload, { new: true })
            res.status(200).json(updatedTrip)
        } else {
            res.status(403).json({ message: 'you dont have rights to edit this trip (not a participant)' })
        }
    } catch (error) {
        console.log(error)
